          ...
          <div className="card mb-6">
              ...
              {vehicle.imageUrl ? (
                <img
                  ...
                  onError={(e) => {
                    ...
                    e.currentTarget.parentElement!.innerHTML = '<span class="text-8xl flex items-center justify-center h-full">🚗</span>'
                  }}
                />
              ) : (
            ...
            <div className="flex gap-2">
              {[1, 2, 3, 4, 5].map((i) => (
                <div
                  key={i}
                  className="w-20 h-20 bg-dark-700 rounded-lg overflow-hidden cursor-pointer hover:border-primary-600 border-2 border-transparent"
                >
                  {vehicle.imageUrl ? (
                    <img
                      src={vehicle.imageUrl}
                      alt={`${vehicle.brand} ${vehicle.model} - View ${i}`}
                      className="w-full h-full object-cover"
                    />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center">
          ...
          <div className="card sticky top-4">
            <div className="mb-6">
              <div className="flex items-baseline gap-2 mb-2">
                <span className="text-3xl font-bold text-white">${vehicle.dailyPrice}</span>
            ...
            {rentalDays > 0 && (
              <div className="bg-dark-700 rounded-lg p-4 mb-6">
                <div className="flex justify-between mb-2">
                  <span className="text-gray-400">${vehicle.dailyPrice} x {rentalDays} days</span>
                  <span className="text-white font-semibold">
                    ${(vehicle.dailyPrice * rentalDays).toFixed(2)}
                  </span>
                </div>
                <div className="flex justify-between mb-2">
                  <span className="text-gray-400">Taxes & Fees</span>
                  <span className="text-white font-semibold">
                    ${(totalPrice - vehicle.dailyPrice * rentalDays).toFixed(2)}
                  </span>
                </div>
